...
class Ticket {
    ...
    function setTicketStatus($ticket_status_id)
    {
        $sql='UPDATE '.TICKET_TABLE.' SET ticket_status_id='
                .db_input($ticket_status_id)
             .' WHERE ticket_id='.db_input($this->getId());
    ...
    function close() {
        ...
        $closed_ticket_status=Ticket_Status::getClosedStatus();
        
        if($closed_ticket_status)
        {
            $this->setTicketStatus($closed_ticket_status->getId());
        }
        $this->reload();
    ...
    function reopen($isanswered=0) {
        ...
        $default_ticket_status=Ticket_Status::getDefaultStatus();
        
        if($default_ticket_status)
        {
            $this->setTicketStatus($default_ticket_status->getId());
        }
   ...
   function onOverdue($whine=true, $comments="") {
    ...
    function update($vars, &$errors) {
        ...
        if(!db_query($sql) || !db_affected_rows())
          ...
          if(!$this->setTicketStatus($vars['ticket_status_id']))
          {
              $errors['ticket_status']='Unable to update ticket status';
          }
   ...
   /*============== Static functions. Use Ticket::function(params); =============nolint*/
    ...
    function create($vars, &$errors, $origin, $autorespond=true, $alertstaff=true) {
        ...
        if(!db_query($sql) || !($id=db_insert_id()) || !($ticket =Ticket::lookup($id)))
          ...
          if($ticket->setTicketStatus($vars['ticket_status_id']))
          {
              $errors['ticket_status']='Unable to update ticket status';
          }
